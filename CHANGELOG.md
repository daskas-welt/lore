...
### Fixed
- Build exe as console app (`console=True` in lore.spec) — `v0.2.2` windowed build left `stdin` as `None`, crashing Textual's Win32 driver on `fileno()` when double-clicked

## [0.2.2] - 2026-09-09

### Fixed
- Bundle all `textual` lazy-loaded widgets (`_markdown_viewer`, `_tabs`, `_list_view`, etc.) in PyInstaller exe — `v0.2.1` crashed with `No module named 'textual.widgets._markdown_viewer'` (lore.spec)
